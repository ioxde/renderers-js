...
>(
...
): AuthorizeNonceAccountInstruction<typeof SYSTEM_PROGRAM_ADDRESS, TAccountNonceAccount, TAccountNonceAuthority> {
    // Program address.
    ...
    return Object.freeze({
        ...
        programAddress,
    } as AuthorizeNonceAccountInstruction<typeof SYSTEM_PROGRAM_ADDRESS, TAccountNonceAccount, TAccountNonceAuthority>);
}
...
>(
    ...
    if (!containsBytes(instruction.data, getU32Encoder().encode(AUTHORIZE_NONCE_ACCOUNT_DISCRIMINATOR), 0)) {
        const error = new Error(
            `parseAuthorizeNonceAccountInstruction: instruction data does not match the AuthorizeNonceAccount discriminator`,
        );
