...
export function parseBurnInstruction<TProgram extends string, TAccountMetas extends readonly AccountMeta[]>(
    ...
    if (!containsBytes(instruction.data, getU8Encoder().encode(BURN_DISCRIMINATOR), 0)) {
        const error = new Error(`parseBurnInstruction: instruction data does not match the Burn discriminator`);
