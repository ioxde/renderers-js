...
export function parseApproveInstruction<TProgram extends string, TAccountMetas extends readonly AccountMeta[]>(
    ...
    if (!containsBytes(instruction.data, getU8Encoder().encode(APPROVE_DISCRIMINATOR), 0)) {
        const error = new Error(`parseApproveInstruction: instruction data does not match the Approve discriminator`);
